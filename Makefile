...
DEFINES += -DVA2PA
DEFINES += -DPROC_TIMES
DEFINES += -DBENNY_MOOTEX
DEFINES += -DOLD

# When you are ready to start development.
DEFINES += -DKTHREADS
#
# You should look for this macro. It is only used once.
DEFINES += -DDTICKS
...
QEMUOPTS = -drive file=fs.img,index=1,media=disk,format=raw -drive file=xv6.img,index=0,media=disk,format=raw -smp $(CPUS) -m 512 $(QEMUEXTRA) $(QEMU_SHUTDOWN)
